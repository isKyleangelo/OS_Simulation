...
class PusoyOSSimulator:
    ...
    def terminate_process(self, pid, signal='SIGTERM'):
        """Terminate a process and release every subsystem resource it owns."""
        ...
        if process.state.name == 'TERMINATED':
            result = {'success': True, 'message': f'Process {pid} already terminated'}
        else:
            result = self.process_manager.kill_process(pid, signal)
    ...
    def launch_application(self, app_id, instance_id=None, singleton=False):
        ...
        result = self.application_manager.launch_application(app_id, instance_id, singleton)
        
        if not result['success']:
            return result

        if result.get('already_running'):
            app = self.application_manager.get_application(
                app_id=app_id,
                instance_id=result.get('app', {}).get('instance_id')
            )
            process = self.process_manager.get_process(app.pid) if app and app.pid else None
            if process:
                result['app'] = app.to_dict()
                result['process'] = process.to_dict()
                return result

            self.application_manager.close_application(app_id=app_id, instance_id=instance_id, force=True)
            result = self.application_manager.launch_application(app_id, instance_id, singleton)
            if not result['success']:
                return result
        ...
        if process:
            ...
            process.instance_id = app.instance_id
            process.process_type = 'application'
            result['app'] = app.to_dict()
            result['process'] = process.to_dict()
            return result
        else:
            self.application_manager.close_application(
                app_id=app_id,
                instance_id=result.get('app', {}).get('instance_id'),
                force=True
            )
    ...
    def close_application(self, app_id=None, force=True, instance_id=None, pid=None):
        """Close an application process or release one browser window."""
        app = self.application_manager.get_application(app_id=app_id, instance_id=instance_id, pid=pid)
        if not app:
        ...
        pid = app.pid
        app_result = self.application_manager.close_application(
            app_id=app.app_id,
            instance_id=app.instance_id,
            pid=pid,
            force=force
        )
        if app_result.get('still_running'):
            return app_result

        if pid:
            self.terminate_process(pid, 'SIGTERM')
    ...
    def step_simulation(self):
        ...
        if next_process:
            ...
            if next_process.state.name == 'TERMINATED':
                if getattr(next_process, 'app_id', None):
                    self.close_application(
                        next_process.app_id,
                        force=True,
                        instance_id=getattr(next_process, 'instance_id', None),
                        pid=next_process.pid
                    )
                else:
                    self.memory_manager.deallocate_memory(next_process.pid)
                    self.scheduler.remove_process_from_queue(next_process)
                    self.process_manager.remove_process(next_process.pid)
