...
def static_asset_version(filename):
    """Return a stable cache-busting token for local static assets."""
...
def inject_static_asset_version():
    """Expose static asset versioning to templates."""
...
def process_opened_at(process):
    """Return the creation time used by desktop process listings."""
    created_at = getattr(process, 'created_at', None)
    if created_at:
        return created_at.isoformat()
    if getattr(process, 'start_time', None):
        return process.start_time.isoformat()
    return None


def simulated_cpu_usage(process=None, app=None):
    """Return a deterministic live-looking CPU value without random UI churn."""
    base = 3
    if app:
        base = float(app.get('cpu_usage_percent', base) or base)
    elif process:
        base = 2 + min(8, max(1, process.memory_required // 64))
    ...
    pid = int(process.pid if process else app.get('pid', 0) if app else 0)
    pulse = ((int(datetime.now().timestamp() * 2) + pid) % 5) - 2
    return round(max(0, min(100, base + pulse)), 1)
...
def calculate_system_metrics():
    ...
    try:
        ...
        app_pids = {app.get('pid') for app in running_apps if app.get('pid') is not None}
        ...
        app_cpu = sum(
            simulated_cpu_usage(process_by_pid.get(app.get('pid')), app)
            for app in running_apps
        )
        background_cpu = sum(
            simulated_cpu_usage(process)
            for process in processes
            if process.pid not in app_pids and process.state.name != 'TERMINATED'
        )
        ...
        metrics = {
            ...
            'cpu_utilization': min(100, app_cpu + background_cpu),
            'memory_used': memory_used,
            'memory_total': memory_total,
            'memory_utilization': memory_percent,
            'avg_wait_time': round(avg_wait_time, 2),
            'avg_turnaround_time': round(avg_turnaround, 2),
            'context_switches': simulator.scheduler.context_switches,
...
def api_scheduling_algorithms():
    ...
    return jsonify({
        'algorithms': [
            ...
            {'name': 'Multilevel Feedback Queue', 'id': 'mlfq', 'description': 'Feedback queues with adaptive time slices'},
            {'name': 'Aging Priority', 'id': 'aging', 'description': 'Priority scheduling with starvation prevention'}
        ]
    })
...
def kill_process():
    ...
    try:
        ...
        if app:
            result = simulator.close_application(
                app.app_id,
        ...
        result = simulator.terminate_process(pid, signal)
...
def get_app_by_pid(pid):
    ...
    return simulator.application_manager.get_application_by_pid(pid)
...
def build_task_manager_snapshot():
    ...
    for app in running_apps:
        ...
        app_tasks.append({
            'type': 'app',
            'instance_id': app.get('instance_id'),
            'app_id': app['app_id'],
            'name': app['name'],
            'pid': app.get('pid'),
            ...
            'window_count': app.get('window_count', 1),
            'active': app.get('active', False),
            'last_active_at': app.get('last_active_at'),
            'cpu_usage': simulated_cpu_usage(process, app),
    ...
    for process in processes:
        ...
        background_tasks.append({
            ...
            'opened_at': process_opened_at(process),
            'window_state': 'hidden',
            'window_count': 0,
            'active': False,
            'last_active_at': None,
            'cpu_usage': simulated_cpu_usage(process),
...
def task_action():
    ...
    try:
        ...
        if action == 'end':
            if pid is not None and get_app_by_pid(pid):
                result = simulator.close_application(
                    app_id=app_id,
                ...
                result = simulator.terminate_process(pid, 'SIGTERM')
            else:
                return jsonify({'success': False, 'message': 'Task not found'}), 404

            if result.get('success'):
                system_metrics['total_processes_terminated'] += 1
                log_event('task_manager_end_task', {'pid': pid, 'app_id': app_id, 'instance_id': instance_id})
            return jsonify(result)
        ...
        if action == 'force_end':
            if pid is not None and get_app_by_pid(pid):
                ...
                result = simulator.terminate_process(pid, 'SIGKILL')
            else:
                return jsonify({'success': False, 'message': 'Task not found'}), 404

            if result.get('success'):
                system_metrics['total_processes_terminated'] += 1
                log_event('task_manager_force_end_task', {'pid': pid, 'app_id': app_id, 'instance_id': instance_id})
            return jsonify(result)
        ...
        if action == 'resume' and process:
            ...
            simulator.scheduler.add_process_to_queue(process)
...
def launch_app():
    ...
    try:
        ...
        result = simulator.launch_application(app_id, instance_id=instance_id, singleton=singleton)
        if result.get('success'):
            if not result.get('already_running'):
                system_metrics['total_processes_created'] += 1
            log_event('app_launched', {
                'app_id': app_id,
                'instance_id': result.get('app', {}).get('instance_id'),
...
def close_app():
    ...
    try:
        ...
        result = simulator.close_application(app_id, instance_id=instance_id, pid=pid, force=force)
        if result.get('success'):
            if not result.get('still_running'):
                system_metrics['total_processes_terminated'] += 1
...
@app.route('/api/app-window-state', methods=['POST'])
def app_window_state():
    """Synchronize browser window state with the process table."""
    try:
        data = request.get_json() or {}
        app_id = data.get('app_id')
        instance_id = data.get('instance_id')
        pid = data.get('pid')
        if not app_id and not instance_id and not pid:
            return jsonify({'success': True, 'message': 'No app state to update'})
        result = simulator.application_manager.update_window_state(
            app_id=app_id,
            instance_id=instance_id,
            pid=pid,
        ...
        log_event('window_state_sync_error', {'error': str(e)})
        return jsonify({'success': False, 'error': str(e)}), 400
