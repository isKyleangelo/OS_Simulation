(() => {
    ...
    const apps = {
        ...
        texteditor: { name: 'Text Editor', icon: 'text', color: '#2563eb', singleton: true, desktop: true },
        imageviewer: { name: 'Image Viewer', icon: 'image', color: '#14a37f', singleton: true, desktop: true },
        pdfviewer: { name: 'PDF Viewer', icon: 'pdf', color: '#d92d20', singleton: true, desktop: true },
        mediaplayer: { name: 'Media Player', icon: 'music', color: '#bd3f83', singleton: true, desktop: true },
    ...
    async function openApp(type, payload = {}, options = {}) {
        ...
        if (backendApps[type]) {
            try {
                launchResult = await api(`${API}/launch-app`, 'POST', { app_id: backendApps[type] });
    ...
    function matchesTaskSearch(task, query) {
        ...
        return [task.name, task.app_id, task.pid, task.state, task.window_state, task.type]
            .filter(Boolean)
    ...
    function closeTaskProcess(pid, appId = null) {
        const win = getWindowByPid(pid) || getWindowByAppId(appId);
        if (!win) return;
        closeWindow(win, { skipBackend: true });
    }
                ...
                <div class="content grid">
                    ...
                    <div class="card">
                        <table class="table">
                            ...
                            `).join('') || '<tr><td colspan="8" class="muted">No tasks.</td></tr>'}</tbody>
                        </table>
                    </div>
                </div>
            </div>`;
